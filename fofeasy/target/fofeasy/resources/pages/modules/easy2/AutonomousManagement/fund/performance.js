...
define(function(require, exports, module) {
	...
	function initConfig() {
		isFirst = true;
		fundId = $("#fundId").val();
		freq = 'w'; //$(".frequencyLdiv .frequencyActive").data('freq');
		initEvent();
		similarRankings();
		incomeIndicators();
		riskIndicators();
	...
	function initEvent() {
		...
		//同类排名选择频率
		$('#similarRankingsSLT').on('change',function(){
			rankFrequency = $(this).val();
			similarRankings();
		})
		//动态回撤日期选择
		...
		$('#showAll').click(function() {
			...
			$.ajax({
				url : apiPath + "/api/v1/self_management/indicators/nav_series/", //return_max_retracement
				type : 'post',
		...
		$('#showAll1').click(function() {
			...
			$.ajax({
				url : apiPath + "/api/v1/self_management/indicators/dynamic_retracement/", //return_max_retracement
				type : 'post',
				contentType : "application/json;charset=utf-8",
				data : JSON.stringify(params),
				success : function(resp) {
	...
	function similarRankings() {
		...
		$.ajax({
			url : apiPath + '/api/v1/self_management/indicators/ranking/',
			type : 'post',
			contentType : "application/json;charset=utf-8",
			data : JSON.stringify(params),
			success : function(resp) {
				if (resp.error_log != undefined) {
					$('#similarRankingsTab').html("");
					$('#similarRankingsGrid').html("");
					$('#similarRankingsTab').html("数据不足，不足以排名");
					$('#similarRankingsGrid').html("数据不足，不足以排名");
				} else {
					...
					gs.percentage_values[rankFrequency].series[0].pointPadding = 0.3;
					gs.percentage_values[rankFrequency].series[1].pointPadding = 0.4;
					gs.percentage_values[rankFrequency].series.pop(); //删除 债券基金
					initchart($('#similarRankingsGrid'), {categories:gs.percentage_values[rankFrequency].categories,series:gs.percentage_values[rankFrequency].series}, {
	...
	function incomeIndicators() {
		...
		$.ajax({
			url : apiPath + '/api/v1/self_management/indicators/return/',
			type : 'post',
			contentType : "application/json;charset=utf-8",
			data : JSON.stringify(params),
			success : function(resp) {
				if (resp.error_log != undefined) {
					...
					initchart($('#incomeBcharts'),g,{'reservations':'percent','chart_type':'column',legend:{enabled:true}});
					//表
	...
	function riskIndicators() {
		...
		$.ajax({
			url : apiPath + '/api/v1/self_management/indicators/risk/',
			type : 'post',
			contentType : "application/json;charset=utf-8",
			data : JSON.stringify(params),
			success : function(resp) {
				if (resp.error_log != undefined) {
					...
					initchart($('#riskScharts'),gs,{'reservations':'percent','chart_type':'column',legend:{enabled:true}})
					dom.find('select').val(benchmark);
	...
	function dynamicRetreat() {
		...
		$.ajax({
			url : apiPath + '/api/v1/self_management/indicators/dynamic_retracement/',
			type : 'post',
			contentType : "application/json;charset=utf-8",
			data : JSON.stringify(params),
			success : function(resp) {
	...
	function riskAdjustment() {
		...
		$.ajax({
			url : apiPath + '/api/v1/self_management/indicators/risk_adjustment/',
			type : 'post',
			contentType : "application/json;charset=utf-8",
			data : JSON.stringify(params),
			success : function(resp) {
				if (resp.error_log != undefined) {
					...
					initchart($('#riskAdjustmentGrid'),gs,{'reservations':'fixed4','chart_type':'column',legend:{enabled:true}})
					dom.find('select').val(benchmark);
	...
	function relativeIndex() {
		...
		$.ajax({
			url : apiPath + '/api/v1/self_management/indicators/relative/',
			type : 'post',
			contentType : "application/json;charset=utf-8",
			data : JSON.stringify(params),
			success : function(resp) {
				if (resp.error_log != undefined) {
					...
					initchart($('#relativeIndexGrid'),gs,{'reservations':'fixed4','chart_type':'column',legend:{enabled:true}});
					var tr_a = $('.tr_a');
	...
	function initAction() {
		...
		var assetUl = document.getElementById('incomeUl');
	 ...
	 * hcharts 净值表格
	 */
	function initNavTable(datas) {
		var jsonData = {
			'data' : datas['data'],
			'dates' : datas['dates_reverse']
		};
		var datas = [];
		...
		mainGrid = $('#nav-main-grid').bootstrapTable({
			showExport : true,
