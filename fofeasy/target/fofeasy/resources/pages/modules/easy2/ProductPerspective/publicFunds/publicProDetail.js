...
define(function(require, exports, module) {
    ...
    var productBasic = require('easy2/ProductPerspective/publicFunds/productBasic');
    var productPerformance = require('easy2/ProductPerspective/publicFunds/productPerformance');
    var positionAnalysis = require('easy2/ProductPerspective/publicFunds/publicPositionsAnalysis');
    var productAttribution = require('easy2/ProductPerspective/publicFunds/productAttribution');
    var productAnalysis = require('easy2/ProductPerspective/publicFunds/productAnalysis');
...
//						$el.find('a').addClass('yellow');
    ...
    function showBasic(){
        $("#main-content").html("");
        $.ajax({
            url:ctx+"/ProductPerspective/pubBasic",
            type:"get",
            data:{},
    ...
    function showNet(){
        $("#main-content").html("");
        $.ajax({
            url:ctx+"/ProductPerspective/pubPerformance",
            type:"get",
            data:{},
            success:function(data){
                $("#main-content").append(data);
    ...
    function showHold(){
        $("#main-content").html("");
        $.ajax({
            url:ctx+"/ProductPerspective/pubPosition",
            type:"get",
            data:{},
            success:function(data){
                $("#main-content").append(data);
                // 绑定事件
                positionAnalysis.init();
            }
        });	}
    /**
     * 归因分析
    ...
    function showAttribution(){
        $("#main-content").html("");
        $.ajax({
            url:ctx+"/ProductPerspective/pubAttribution",
            type:"get",
            data:{},
            success:function(data){
                $("#main-content").append(data);
    ...
    function showSituational(){
        $("#main-content").html("");
        $.ajax({
            url:ctx+"/ProductPerspective/pubAnalysis",
            type:"get",
            data:{},
            success:function(data){
                $("#main-content").append(data);
...
//				'frequency':conditionDatas.frequency,  //净值
    ...
    function initPrcinfo(){
        ...
        $.ajax({
            ...
            success:function(res) {
                if (res.success) {
                	//区分货币型
                	if(res.records.fund_type=="0204"){
                		$("#main").css("display","none");
                        $("#coin").css("display","flex");
                        $(".nhb").css("display","none");
                        $("#menuUl").css("display","block");
					}else{
                        $("#main").css("display","flex");
                        $("#coin").css("display","none");
                        $("#nhb").css("display","inline-block");
                        $("#menuUl").css("display","block");
					}
					//区分股票型和偏股混合型，可有归因分析
					if(res.records.fund_type!="0201"&&res.records.fund_stype!="020301"){
                        $('#menuImg3').attr('data-isLegal', 'false');
                    }
                    var resp = res.records;
                    document.title = resp.fund_name;
                    sessionStorage.setItem("fund_name",resp.fund_name);
                    showNet();
                    // showHold();
                    $('#Policy').text(resp.fund_type_name ? resp.fund_type_name:"--");
                    ...
                    $('#org_name2').text(resp.org_name);
                    $('#org_name').attr('data-content', resp.org_name);
                    $('#org_name2').attr('data-content', resp.org_name)
                    $('#netNav').text(util.fmtFixed(resp.latest_nav,4));
                    $('#added_nav').text(util.fmtFixed(resp.added_nav,4));
                    $('#swanav').text(util.fmtFixed(resp.swanav,4));
                    $('#sdate').text(resp.sdate);
                    $('#nav_date').text(resp.latest_nav_date);
                    $('#nav_date2').text(resp.latest_nav_date);
                    $('#m1_return').text(util.fmtRatio(resp.m1_return));
                    $('#year_return').text(util.fmtRatio(resp.year_return));
                    $('#total_return').text(util.fmtRatio(resp.total_return));
                    $('#d7_return_a').text(util.fmtRatio(resp.d7_return_a));
                    $('#return_10k').text(util.fmtFixed(resp.return_10k,4));
                    // $('#d7_return_a_rank').text(resp.d7_return_a_rank?resp.d7_return_a_rank.rank.d7_return_a:"");
                    // $('#d7_return_a_rank_total').text(resp.d7_return_a_rank?resp.d7_return_a_rank.total:"");
                    $('#fundStatu').text(resp.fund_status);
                    $('#foundation_date').text(resp.foundation_date?resp.foundation_date:"--");
                    $('#foundation_date2').text(resp.foundation_date?resp.foundation_date:"--");
                    ...
                    if(resp.person_info.length>0){
                        ...
                        $('#fundManeger').html(member);
                        $('#person_name').html(member);
                    }else{
                        $('#fundManeger').html("--");
                        $('#person_name').html("--");
                    }
    ...
    function yieldContribution(){
        ...
        $.ajax({
            url:apiPath + "/api/v1/mutual/external_attribution/income_compose/",
            type:'post',
            contentType:"application/json;charset=utf-8",
            data:JSON.stringify(params),
            success:function(resp){
    ...
    function riskContribution(){
        ...
        $.ajax({
            url:apiPath + "/api/v1/mutual/external_attribution/risk_compose/",
            type:'post',
            contentType:"application/json;charset=utf-8",
            data:JSON.stringify(params),
            success:function(resp){
    ...
    function factorsCorrelation(){
        ...
        $.ajax({
            url:apiPath + "/api/v1/mutual/external_attribution/correlation/",
            type:'post',
            contentType:"application/json;charset=utf-8",
            data:JSON.stringify(params),
            success:function(resp){
