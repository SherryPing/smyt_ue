...
define(function(require, exports, module) {
    ...
    function fit(){
        ...
        $.ajax({
            url:apiPath + "/api/v1/mutual/external_attribution/fit/",
            type:'post',
	...
	function yieldContribution(){
		...
		$.ajax({
			...
			success:function(resp){
                    $('#starDate1').text(resp.section_date_range.min);
                    $('#endDate1').text(resp.section_date_range.max);
					$('#starDate3').text(resp.section_date_range.min);
					$('#endDate3').text(resp.section_date_range.max);
                    composeCharts($('#returnsColumn'), resp)
                    initSpider($('#returnsSpider'), resp)
			...
			error:function(){
                $("#returnsColumn").html("<p class='tip' style='position:relative;top: 42%;text-align: center;'>暂无数据</p>")
                $("#returnsSpider").html("<p class='tip' style='position:relative;top: 42%;text-align: center;'>暂无数据</p>")
			}
		})
	}
	//风险贡献分解
	function riskContribution(){
		...
		$.ajax({
			...
			success:function(resp){
                ...
                $("#riskColumn").html("<p class='tip' style='position:relative;top: 42%;text-align: center;'>暂无数据</p>")
                $("#riskSpider").html("<p class='tip' style='position:relative;top: 42%;text-align: center;'>暂无数据</p>")
			}
		})
	}
	//因子相关性
		...
		$.ajax({
			...
			success:function(resp){
				if(resp.succeed){
                    ...
                    $("#correlationTbl").html("<p class='tip' style='position:relative;top: 42%;text-align: center;'>"+resp.msg+"</p>")
				}
			},error:function(){
                $("#correlationTbl").html("<p class='tip' style='position:relative;top: 42%;text-align: center;'>暂无数据</p>")
			}
		})
	}
	//收益贡献分解条形图
    ...
    $(document).on("click","#tabs ul li",function(e){
        ...
        if(flag == 'true'){
            ...
            layer.msg('目前仅提供股票型基金和偏股混合型基金的三因子模型！ ');
    ...
    function initShapeChartData(){
        ...
        $.each(records,function(i,e){
            plicy = $("#Policy").text() == "股票型基金";
            if(e.isChart && typeof e.hideLayer == 'object' && !plicy){
    ...
    function initTable(options) {
	    var params = {
            ...
            onPostBody : function($row){
                ...
                // var _selfTab = $(options.el).attr("id");
                // if($('span[data-table="'+_selfTab+'"]').length){
                //     return
                // }
                // var _uid = _selfTab + "-group";
                // $("#"+_uid+" ul.pagination").after("<div class='goPage'><input class='percent pageNum1' type='number' value='' t_value='' o_value='' data-input='"+_selfTab+"'><span class='goNum1' data-table='"+_selfTab+"'>GO</span></div>");
                // $("#"+_uid+" .fixed-table-pagination").css('text-align','center');
                // $("#"+_uid+" .pagination-detail").removeClass('pull-left');
                // $("#"+_uid+" .pagination-detail").css('display','inline-block');
                // $("#"+_uid+" div.pagination").removeClass('pull-right');
                // $("#"+_uid+" div.pagination").addClass('pagina1');
                // $("#"+_uid+" ul.pagination").addClass('pagina');
                //如果配置回调函数，将在这里执行
                if(options.fn){
                    var event = eval(options.fn);
                    event(options.res)
